function callbackfunction(status, resulttable) {
    ...
    var stat = null;
    var httpresp = null
    if (resulttable != null) httpresp = resulttable.httpresponse;
    if (httpresp != null) stat = httpresp["integrityStatus"];
...
function asyncServiceCall(invokeTwice) {
    try {
        ...
        if (contentType == "json") inputParamTable["httpheaders"] = httppostreqhdrs
        ...
        if (invokeTwice == "true") kony.net.invokeServiceAsync(url, inputParamTable, callbackfunction);
    } catch (err) {
        alert("Error" + err);
    }
}
...
function callbackfunctionAsyncHTTP(request) {
    ...
    if (state == constants.HTTP_READY_STATE_DONE) {
        ...
        if ((httpresp != null) && (respType == "json")) httpresp = JSON.stringify(httpresp);
...
function httpRequestSend(isAsync, invokeTwice) {
    try {
        ...
        if (contentType == "json") request.setRequestHeader("Content-Type", "application/json");
...
function addKeyVal() {
    if (keyValsTbl == null) keyValsTbl = {}
    var k = frmHttp.tbKey.text
    var v = frmHttp.tbValue.text
    if (k != null) k = k.trim()
    if (v != null) v = v.trim()
    if (k != "" && v != "") keyValsTbl[k] = v
    else {
...
function removeKeyVal() {
    if (keyValsTbl == null) return;
    var k = frmHttp.tbKey.text
    if (k != "") delete keyValsTbl[k]
    frmHttp.tbKey.text = ""
    frmHttp.tbValue.text = ""
}
